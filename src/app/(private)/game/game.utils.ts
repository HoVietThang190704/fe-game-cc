import type { GameCell, GameBoard } from "./game.types";

const ROWS = 10;
const COLS = 10;
const BOARD_SIZE = 100;

// Tạo cell ID dựa trên row, col
export function getCellId(row: number, col: number): string {
  return `${row}-${col}`;
}

// Parse cell ID để lấy row, col
...
// Đặt mìn vào các vị trí được chọn
export function placeMines(board: GameBoard, mineCells: string[]): GameBoard {
  ...
  // Tính toán adjacent mines cho tất cả cells
...
// Reveal ô (click trên bảng của đối thủ)
export function revealCell(board: GameBoard, cellId: string): { board: GameBoard; result: "hit" | "miss" } {
  ...
  // Nếu adjacent mines = 0, reveal xung quanh
...
// Kiểm tra xem game có thắng không (tất cả mìn được reveal)
